    ...
    <div class="content-card">
        ...
        <div class="content">
            <div class="breadcrumbs">
                <a href="index.php"> Главная</a> / <span>Список заявок</span>
            </div>

            <h2>Список заявок на полиграфическую продукцию</h2>

            <p style="margin-bottom: 15px; font-size: 0.9rem; color: #6c7a89;">
                 Найдено: <?= count($requests) ?>
                <?php
                    ...
                    <table>
                        ...
                        <tbody>
                            <?php foreach ($requests as $row): ?>
                                <?php
                                $is_overdue = ($row['deadline'] < date('Y-m-d') && $row['status'] != 'Готово');
                                ...
                                <tr class="<?= $row_class ?>">
                                    ...
                                    <td>
                                        ...
                                        switch($row['status']) {
                                            ...
                                            case 'В работе': $status_class = 'status-work'; $status_text = 'В работе'; break;
                                            case 'Готово': $status_class = 'status-done'; $status_text = 'Готово'; break;
                                            default: $status_class = 'status-new'; $status_text = $row['status'];
                                        }
                                        ?>
                                    ...
                                    <?php if ($_SESSION['role'] == 'admin'): ?>
                                        <td class="action-buttons">
                                            <a href="edit.php?id=<?= $row['id'] ?>" class="btn-edit" title="Редактировать заявку"> Редактировать</a>
                                            <a href="delete.php?id=<?= $row['id'] ?>" class="btn-delete" title="Удалить заявку" onclick="return confirm('Удалить заявку?')"> Удалить</a>
                                        </td>
                                    <?php else: ?>
                                        <td></td>
                                    <?php endif; ?>
